...
/**
 * Created by Shreyash on 23-03-2018.
 */
 ...
 */
public class DrawView2 extends SurfaceView {
    ...
    protected void onDraw(Canvas canvas){
        ...
        for(int i = 0; i < size; i++){
            ...
            else paint.setColor(REFERENCE_POINT_TWO_COLOR);
...
/*case MotionEvent.ACTION_MOVE:{
    ...
    public double calculate2(double reference, int inputUnitIndex, int outputUnitIndex){
        if(circlePoints.size() != 4){
            Toast.makeText(context, getResources().getString(R.string.error_noPoints), Toast.LENGTH_SHORT).show();
            return -1;
        }
        return Ruler.compute2(circlePoints, reference, inputUnitIndex, outputUnitIndex);
    }

    /*public double calculateHeight2(double reference, int inputUnitHeightIndex, int outputUnitHeightIndex){
        if(circlePoints.size() != 8){
            Toast.makeText(context, getResources().getString(R.string.error_noPoints), Toast.LENGTH_SHORT).show();
            return -1;
        }
    ...
    public double calculateWidth2(double reference, int inputUnitWidthIndex, int outputUnitWidthIndex){
        if(circlePoints.size() != 8){
            Toast.makeText(context, getResources().getString(R.string.error_noPoints), Toast.LENGTH_SHORT).show();
            return -1;
        }
